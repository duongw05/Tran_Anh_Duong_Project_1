...
public class ThemKichThuocJFrame extends javax.swing.JFrame {
    ...
    private ThuocTinhSanPham getFormDataThuocTinhSP() {
        ...
        tenKichThuoc = tenKichThuoc.split(" ")[0];
        ...
        if (!tenKichThuoc.matches("^[a-zA-Z]+$")) {
            JOptionPane.showMessageDialog(null, "Tên kích thước không được chứa số hoặc ký tự đặc biệt", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }

        // Kiểm tra độ dài của tên kích thước
        if (tenKichThuoc.length() > 10) {
            JOptionPane.showMessageDialog(null, "Tên kích thước không được vượt quá 10 ký tự", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }   
