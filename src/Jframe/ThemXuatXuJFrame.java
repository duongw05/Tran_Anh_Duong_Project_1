...
//        setupKeyListener();
    ...
    private ThuocTinhSanPham getFormDataThuocTinhSP() {
        ...
        // Lấy từ đầu tiên của tên xuất xứ
        ...
        // Kiểm tra xem tên xuất xứ có chứa số hoặc ký tự đặc biệt không
        if (!tenXuatXu.matches("^[\\p{L}\\s]+$")) {
            JOptionPane.showMessageDialog(null, "Tên xuất xứ không được chứa số hoặc ký tự đặc biệt", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }

        // Kiểm tra độ dài của tên xuất xứ
        if (tenXuatXu.length() > 30) {
            JOptionPane.showMessageDialog(null, "Tên xuất xứ không được vượt quá 30 ký tự", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }
