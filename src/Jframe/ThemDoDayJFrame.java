...
public class ThemDoDayJFrame extends javax.swing.JFrame {
    ...
    private DoDayRepository repoDoDay = new DoDayRepository();
    ...
    private ThuocTinhSanPham getFormDataThuocTinhSP() {
        String tenDoDay = txt_TenDoDay.getText().trim();

        // Lấy từ đầu tiên của tên độ dày
        tenDoDay = tenDoDay.split(" ")[0];
        ...
        // Kiểm tra xem tên độ dày có chứa số hoặc ký tự đặc biệt không
        if (!tenDoDay.matches("^[\\p{L}\\s]+$")) {
            JOptionPane.showMessageDialog(null, "Tên độ dày không được chứa số hoặc ký tự đặc biệt", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }

        // Kiểm tra độ dài của tên độ dày
        if (tenDoDay.length() > 20) {
            JOptionPane.showMessageDialog(null, "Tên độ dày không được vượt quá 20 ký tự", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }

        if (repoDoDay.isDoDayExist(tenDoDay)) {
