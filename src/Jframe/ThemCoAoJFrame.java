...
public class ThemCoAoJFrame extends javax.swing.JFrame {
    ...
    private ThuocTinhSanPham getFormDataThuocTinhSP() {
        String tenCoAo = txt_TenCoAo.getText().trim();
        tenCoAo = tenCoAo.split(" ")[0];
        ...
        if (!tenCoAo.matches("^[\\p{L}\\s]+$")) {
            JOptionPane.showMessageDialog(null, "Tên cổ áo không được chứa số hoặc ký tự đặc biệt", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }

        // Kiểm tra độ dài của tên cổ áo
        if (tenCoAo.length() > 20) {
            JOptionPane.showMessageDialog(null, "Tên cổ áo không được vượt quá 20 ký tự", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }

        // Kiểm tra xem tên cổ áo đã tồn tại chưa
        if (coao.isTenCoAoExist(tenCoAo)) {
            JOptionPane.showMessageDialog(null, "Tên cổ áo đã tồn tại", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }
