...
//        setupKeyListener();
    ...
    private ThuocTinhSanPham getFormDataThuocTinhSP() {
        ...
        // Lấy từ đầu tiên của tên phong cách
        tenPhongCach = tenPhongCach.split(" ")[0];
        ...
        // Kiểm tra xem tên phong cách có chứa số hoặc ký tự đặc biệt không
        if (!tenPhongCach.matches("^[\\p{L}\\s]+$")) {
            JOptionPane.showMessageDialog(null, "Tên phong cách không được chứa số hoặc ký tự đặc biệt", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }

        // Kiểm tra độ dài của tên phong cách
        if (tenPhongCach.length() > 20) {
            JOptionPane.showMessageDialog(null, "Tên phong cách không được vượt quá 20 ký tự", "Lỗi Nhập Liệu", JOptionPane.ERROR_MESSAGE);
            return null;
        }
